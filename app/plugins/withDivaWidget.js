...
const WIDGET_FILES = ['DivaWidget.swift', 'DivaIntents.swift', 'MascotImage.swift'];
...
<dict>
  ...
  <array>
    ...
    let widgetTarget = xcodeProject.pbxTargetByName(WIDGET_TARGET_NAME);
    const targetExists = !!widgetTarget;

    // 5. Add widget extension target if not exists
    if (!targetExists) {
      console.log(`[DivaWidget] Adding widget extension target...`);
      widgetTarget = xcodeProject.addTarget(
        WIDGET_TARGET_NAME,
        'app_extension',
        WIDGET_TARGET_NAME,
        BUNDLE_ID
      ...
      console.log(`[DivaWidget] Target exists, updating files...`);
    }
