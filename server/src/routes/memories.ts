...
export async function memoriesRoutes(app: FastifyInstance) {
  ...
  // Delete ALL memories for a user (RGPD)
  app.delete('/api/v1/memories', {
    ...
    if (error) {
      app.log.error({ msg: 'Failed to clear memories', error });
      return { deleted: 0 };
    }

    return { deleted: count ?? 0, message: 'Toutes les mémoires ont été supprimées' };
  });
}
