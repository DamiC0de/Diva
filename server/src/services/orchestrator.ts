...
Si tu ne connais pas le scheme exact, utilise une URL https:// qui ouvrira Safari.`,
  ...
  {
    name: 'get_glucose',
    description: "OBLIGATOIRE quand l'utilisateur mentionne 'glucose', 'glucose.press', ou demande des articles/news/dossiers/actu. Ce tool accède aux analyses comparatives multi-sources de glucose.press. Chaque analyse croise plusieurs médias internationaux sur un même sujet. Utilise-le À LA PLACE de web_search pour tout ce qui concerne glucose.",
    input_schema: {
      type: 'object' as const,
      properties: {
        limit: {
        ...
        full_content: {
          type: 'boolean',
          description: "Si true, retourne le contenu COMPLET du premier article trouvé. Utilise quand l'utilisateur veut LIRE un article.",
        },
      },
    },
  },
...
export class Orchestrator {
  ...
  private async processTextRequest(
    ...
    try {
      ...
      if (isGlucoseQuery) {
        ...
        // Search comparisons only (the content visible on glucose.press)
        const comparisons = searchTerm 
          ? await searchComparisons(searchTerm, 5) 
          : await getLatestComparisons(5);
        
        if (comparisons.length > 0) {
          let glucoseContext = '[DONNÉES GLUCOSE.PRESS — Analyses comparatives multi-sources]\n\n';
          ...
          if (wantsFull && comparisons.length > 0) {
            ...
            glucoseContext += `📊 ${c.title}\n(${c.sources_count} sources de ${c.countries_count} pays — ${date})\n\n${c.content || c.summary || 'Contenu non disponible'}`;
          } else {
  ...
  private cleanForTTS(text: string): string {
    return text
      ...
      .replace(/__([^_]+)__/g, '$1')
      .replace(/_([^_]+)_/g, '$1')
      // Remove ALL remaining asterisks (orphans, unclosed, etc.)
      .replace(/\*/g, '')
      // Markdown headings
      .replace(/#{1,6}\s/g, '')
      // Bullet points
      .replace(/^[-•]\s/gm, '')
      // Numbered lists — keep the number for natural speech
      .replace(/^(\d+)\.\s/gm, '$1, ')
      ...
      // Special chars that TTS might read aloud
      .replace(/[~|>]/g, '')
      // Clean up whitespace
      .replace(/\n{3,}/g, '\n\n')
      .replace(/  +/g, ' ')
      ...
      try {
        switch (tool.name) {
          ...
          case 'get_glucose': {
            ...
            if (comparisons.length === 0) {
              results.push({ name: tool.name, result: 'Aucune analyse trouvée sur Glucose pour cette recherche.' });
            } else if (compParams.full_content) {
              const c = comparisons[0];
              const date = new Date(c.created_at).toLocaleString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' });
              results.push({ 
                name: tool.name, 
              ...
              results.push({ 
                name: tool.name, 
                result: `${comparisons.length} analyses comparatives depuis glucose.press:\n\n${formatted}\n\n[Source: glucose.press]`
              });
            }
            break;
          }
